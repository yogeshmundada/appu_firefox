...
function fetch_report(response) {
    ...
    self.port.emit("get_text_report_by_number", {
	'type' : "get_text_report_by_number",
...
function register_message_listeners() {
    self.port.on("get-text-report-number_response", fetch_report);
    self.port.on("get_text_report_by_number_response", process_report);
}

register_message_listeners();

setTimeout(function() {
	self.port.emit("get-text-report-number", {
