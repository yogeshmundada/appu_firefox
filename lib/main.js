...
function register_message_listeners() {
    ...
    appu_menu_panel.port.on("open-sign-in", function() {
	    var sign_in_worker = undefined;
	    var sign_in_worker_port = undefined;

	    appu_menu_panel.hide();
	    tabs.open({
		    url: data.url("sign_in.html"),
			onOpen: function(tab) {
			sign_in_worker = tab.attach({
				contentScriptFile: [
						    data.url("thirdparty/jquery-ui-1.9.1.custom/js/jquery-1.8.2.js"),
			...
			sign_in_worker_port = sign_in_worker.port;
		    },
...
// 						    ]
// 			    });
			//console.log("Here here: signinworker: " + JSON.stringify(sign_in_worker));
			console.log("Here here: signinworker: " + JSON.stringify(Object.keys(sign_in_worker)));
			console.log("Here here: Sending message that sign-in-document-ready");
			sign_in_worker.port.emit("sign-in-document-ready");
			console.log("Here here: At the end of onReady()");
		    }
		});	    
	});
}
