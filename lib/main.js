...
hw.onmessage = function(event) {
    my_log("Here here: callback ", new Error);
    my_log("Here here: Answer is: " + JSON.stringify(event.data), new Error);
};

my_log("Here here: before posting message ", new Error);
hw.postMessage({
	'limit' : 10,
	    'cmd' : 'hash',
	    'pwd' : 'secret',
	    });
my_log("Here here: after posting message ", new Error);