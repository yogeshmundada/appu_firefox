...
function sign_in(sender_worker, username, password) {
    ...
    var r = request({
		...
		console.log("Here here: Comepleted my_sign_in response");
		console.log("Here here: Response status: " + response.status);
		console.log("Here here: Response status txt: " + response.statusText);
		console.log("Here here: Response text: " + response.text);
		console.log("Here here: Response json: " + response.json);
		console.log("Here here: Response headers: " + response.headers);
				...
				tabs.open({
					url: data.url("sign_in.html"),
					    ...
					    sign_in_worker.port.on('sign-in', function(args) {
						    console.log("Here here: Got sign-in");
						    bg_sign_in.sign_in(sign_in_worker,
								       args['username'],
								       args['password']);
						});

					    bg_sign_in.testing_msg_sending(sign_in_worker);
					}
				    });	    
			    });
		    }
...
function my_sign_in(username, password) {
    //zero out pii_vault first if guid is differnt
    var wr = { 
	'guid': globals.pii_vault.guid, 
	'username': CryptoJS.SHA1(username).toString(), 
	'password' : CryptoJS.SHA1(password).toString(),
	'version' : globals.pii_vault.config.current_version
    }
...
//globals.reset_pii_vault();

timers.setTimeout(function() {
	vault.test_pii_vault_modify();
	test1.test_pii_vault_get_value();
	console.log("Here here: New value is: " + globals.pii_vault.new_test_vaule);
	globals.reset_pii_vault();
	console.log("Here here: New value is: " + JSON.stringify(globals.pii_vault));
    }, 5000);
