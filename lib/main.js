...
function get_pwd_group(domain, full_hash, password_strength) {
    ...
    for (var g in cr_pwd_groups) {
	my_log("Here here: Sites: " + JSON.stringify(cr_pwd_groups[g]), new Error);
	...
	cr_pwd_groups[current_group].sites = pwd_groups[current_group].sites.slice(0);
...
function open_reports_tab() {
    tabs.open({
		...
		var report_worker = tab.attach({
			...
			contentStyleFile: [
					   data.url("thirdparty/bootstrap/css/bootstrap.css"),
					   data.url("thirdparty/bootstrap/css/bootstrap-responsive.css"),
					   data.url("thirdparty/bootstrap/css/docs.css"),
					   data.url("thirdparty/DataTables-1.9.4/media/css/demo_page.css"),
					   data.url("thirdparty/DataTables-1.9.4/media/css/demo_table.css"),
					   data.url("report.css")
			       ],

		    });
		
		var my_tab_id = report_worker.tab.id;
		report_tab_ids.push(tab.id);
		
		report_worker.port.on("report_user_approved", function(message) {
			my_log("Here here: report_user_approved", new Error);
			schedule_report_for_sending(message.report_number);
		    });

		report_worker.port.on("report_time_spent", function(message) {
			my_log("Here here: report_time_spent", new Error);
		...
		report_worker.port.on("report_tab_opened", function(message) {
			my_log("Here here: report_tab_opened", new Error);
			var sender = { 'tab': {}};
			sender.tab.id = my_tab_id;
		...
		report_worker.port.on("report_tab_closed", function(message) {
			my_log("Here here: report_tab_closed", new Error);
			var sender = { 'tab': {}};
			sender.tab.id = my_tab_id;
		...
		report_worker.port.on("delete_entry", function(message) {
			my_log("Here here: delete_entry", new Error);
			purge_report_entry(message.report_number, message.table_name, message.entry_key);
		    });
		
		report_worker.port.on("get_report_by_number", function(message) {
			my_log("Here here: get_report_by_number", new Error);
			var resp = pii_get_report_by_number(message.report_number);
			var response_report = resp[0];
			var original_report = resp[1];
			response_report.report_number = message.report_number;
			response_report.do_render = message.do_render;
			...
			report_worker.port.emit("get_report_by_number_response", response_report);	    
			original_report.report_updated = false;
		    });


		report_worker.port.on("show-text-report", function(message) {
			var curr_report_id = message.reportnumber;
			tabs.open({
				url: data.url("text_report.html"),
				    onReady: function(tab) {
				    var text_report_worker = tab.attach({
					    contentScriptFile: [
								data.url("thirdparty/jquery-ui-1.9.1.custom/" + 
									 "js/jquery-1.8.2.js"),
								data.url("thirdparty/bootstrap/js/bootstrap.min.js"),
								data.url("thirdparty/DataTables-1.9.4/media/" + 
									 "js/jquery.dataTables.js"),
								data.url("thirdparty/DataTables-1.9.4/media/js/paging.js"),
								data.url("text_report_utility.js"),
								data.url("text_report.js"),
								],
					    contentStyleFile: [
							       data.url("text_report.css")
							       ],

					});
		

				    text_report_worker.port.on("get_text_report_by_number", function(message) {
					    my_log("Here here: get_text_report_by_number", new Error);
					    var resp = pii_get_report_by_number(message.report_number);
					    var response_report = resp[0];
					    var original_report = resp[1];
					    response_report.report_number = message.report_number;
					    response_report.do_render = message.do_render;
					    response_report.num_total_report = pii_vault.past_reports.length + 1;
					    response_report.pi_field_value_identifiers = 
						pii_vault.aggregate_data.pi_field_value_identifiers;
					    text_report_worker.port.emit("get_text_report_by_number_response", 
									 response_report);
					    original_report.report_updated = false;
					});

				    text_report_worker.port.on("get-text-report-number", function(message) {
					    my_log("Here here: get-text-report-number", new Error);

					    text_report_worker.port.emit("get-text-report-number_response", {
						    'reportnumber' : curr_report_id
							});	    
					});
				}
			    });
...
function register_worker_message_listeners(worker) {
    ...
    worker.port.on("check_pending_warning", function(message) {
	    ...
	    r.id = sender.tab.id;
	    my_log("Here here: check_pending_warning RESPONSE: " + JSON.stringify(r), new Error);
